...
pub fn detect_market<'a>(
    _token_mint: Pubkey,
    _quote_mint: Pubkey,
    acc_iter: &mut std::slice::Iter<'a, AccountInfo<'a>>,
) -> Result<DexChoice<'a>, ProgramError> {
    ...
    if maybe_flag.key.to_bytes()[0] % 2 == 0 {
        ...
        super::raydium_v4::collect_accounts(acc_iter).map(|accounts| DexChoice::RaydiumV4 { accounts })
    } else {
        msg!("routing: prefer Meteora");
        super::meteora_v1::collect_accounts(acc_iter).map(|accounts| DexChoice::MeteoraV1 { accounts })
    }
}
